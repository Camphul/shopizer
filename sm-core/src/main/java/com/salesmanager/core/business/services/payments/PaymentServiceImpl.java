...
public class PaymentServiceImpl implements PaymentService {
    ...
    public List<PaymentMethod> getAcceptedPaymentMethods(MerchantStore store) throws ServiceException {
        ...
        for (Map.Entry<String, IntegrationConfiguration> configEntry : modules.entrySet()) {
            IntegrationConfiguration config = configEntry.getValue();
            if (config.isActive()) {
    ...
    public IntegrationConfiguration getPaymentConfiguration(String moduleCode, MerchantStore store) throws ServiceException {
        ...
        if (configuredModules != null && configuredModules.containsKey(mod)) {
        	return configuredModules.get(mod);
        }
    ...
    public Transaction processPayment(Customer customer,
        ...
        if (transactionType == null) {
            TransactionType paymentTransactionType = payment.getTransactionType();
            if (paymentTransactionType != null && paymentTransactionType.equals(TransactionType.CAPTURE)) {
        ...
        if (transactionType == TransactionType.AUTHORIZECAPTURE) {
            ...
            if (payment.getPaymentType().equals(PaymentType.MONEYORDER)) {
    ...
    private void validateCreditCardNumber(String number, CreditCardType creditCard)
        ...
        } else if (CreditCardType.VISA.equals(creditCard)) {
            if ((number.length() != 13 && number.length() != 16)
                    || Integer.parseInt(number.substring(0, 1)) != 4) {
                ServiceException ex = new ServiceException(ServiceException.EXCEPTION_VALIDATION, "Invalid card number", "messages.error.creditcard.number");
                throw ex;
            }
        } else if (CreditCardType.AMEX.equals(creditCard)) {
            if (number.length() != 15
        ...
        } else if (CreditCardType.DINERS.equals(creditCard)) {
            if (number.length() != 14
        ...
        }else if (CreditCardType.DISCOVERY.equals(creditCard)) {
            if (number.length() != 16
