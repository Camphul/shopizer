...
public class ShoppingCartTest extends com.salesmanager.test.common.AbstractSalesManagerCoreTestCase {
	...
	private ProductAttribute blackAttribute;
	@Before
	@Override
	...
	public void populateProducts() throws ServiceException {
		...
		//black
		blackAttribute = new ProductAttribute();
	...
	public void close() throws ServiceException {
		super.close();
		System.out.println("** Resetting test.");
		this.blackAttribute = null;
		this.product = null;
		this.store = null;
	}

	public ShoppingCartItem createShoppingCartItem(ShoppingCart shoppingCart) throws ServiceException {
	...
	//@Test
	public void addItemToShoppingCart() throws ServiceException {
		...
		ShoppingCartItem expectedItem = createShoppingCartItem(shoppingCart);
		shoppingCart.getLineItems().add(expectedItem);


		System.out.println("Before create cart");

		shoppingCartService.create(shoppingCart);
		System.out.println("Retrieving and validating cart.");
		ShoppingCart retrievedCart = shoppingCartService.getByCode(cartCode.toString(), store);
		Assert.assertNotNull("Retrieved cart is null.", retrievedCart);
		Assert.assertEquals("Retrieved cart does not contains exactly 1 line item.",1, retrievedCart.getLineItems().size());
		Assert.assertTrue("Retrieved cart does not contain added line item", retrievedCart.getLineItems().contains(expectedItem));
	}
